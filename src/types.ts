...
export class SuggestionItem extends vscode.TreeItem {
  constructor(public readonly suggestion: Suggestion) {
    ...
    if (suggestion.suggestionType === SuggestionType.forum) {
      this.command = {
      ...
      this.iconPath = {
        light: path.join(__filename, "..", "..", "resources", "biconomy.svg"),
        dark: path.join(__filename, "..", "..", "resources", "biconomy.svg"),
      };
    } else {
      this.command = {
        command: BAAI_OPEN_FILE_COMMAND_ID,
